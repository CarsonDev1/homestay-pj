...
import { Autocomplete, Box, Button, Collapse, Divider, IconButton, Stack, TextField, Typography } from '@mui/material';
...
import { getAllHotel, getAllHotelNameToSearch, searchHotelByCity } from '../Home.Api';
...
const HotelsListPage = () => {
	...
	const listHotels = data;

	console.log('listHotels', listHotels);
	...
	const filterHotels = (hotels) => {
		return hotels?.filter((hotel) => {
			const subServiceMatch =
				...
				hotel?.homeStayService?.some((service) =>
					service?.homeStaySubService?.some((subService) => selectedSubServices?.includes(subService?.name))
				);

			const standardMatch = selectedStandards?.length === 0 || selectedStandards?.includes(hotel?.hotelStandar);

			const priceMatch =
				(minPrice === '' || (hotel?.rooms && hotel?.rooms[0]?.price >= Number(minPrice))) &&
				(maxPrice === '' || (hotel?.rooms && hotel?.rooms[0]?.price <= Number(maxPrice)));
					...
					{data?.length > 0 ? (
						<>
							<Box className='filter__container'>
								...
								<Divider sx={{ width: '100%', height: 0.5, bgcolor: themeColors.gray }} />
								...
								<Box className='filter__amenities'>
									<Typography className='amenities__title'>Amenities</Typography>
									...
									{dataHotelAmenities.map((service, index) => (
										<Box key={index} sx={styles.amenitiesWrapper}>
											...
											>
												<Typography sx={styles.serviceType}>{service?.serviceType}</Typography>
							...
							<Box className='list__container'>
								{(listHotelsBySearch?.length === 0 ? filteredHotels : filteredSearchHotels)?.map(
									(hotel, index) => {
										if (index > hotelPage) {
											return null;
										} else {
								...
								<Box display='flex'>
									{hotelPage <
									(listHotelsBySearch?.length === 0 ? filteredHotels : filteredSearchHotels)?.length -
										1 ? (
										<Box
						...
						<Box width='100%' p='20px 0' display='flex' justifyContent='center' alignItems='center'>
