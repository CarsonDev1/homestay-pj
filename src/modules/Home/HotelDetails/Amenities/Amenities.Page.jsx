	...
	return (
		<>
			{data?.length > 0 ? (
				...
				>
					{data?.map((item, index) => (
						<Box
							...
							key={item?.serviceID}
						>
							<Box display='flex' flexDirection='column' gap='1.5rem' padding='5px 30px'>
								...
								<Box display='flex' gap='2rem' alignItems='center' flexWrap='wrap'>
									{item?.subServices?.map((items) => (
										<Box display='flex' gap='1rem' alignItems='center' key={items?.subServiceID}>
				...
				<Box width='100%' p='20px 60px' display='flex' justifyContent='center' alignItems='center'>
